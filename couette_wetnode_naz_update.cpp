  ...
  for (int t = 0; t < t_steps; t++) {
    ...
    for (int i = 0; i < x_len; i++) {
      for (int j = 0; j < y_len; j++) {
        ...
        for (int k = 0; k < q_num; k++) {
          f_eq[i][j][k] = wk[k] * rho[i][j] * (1 + (ux[i][j] * cx_float[k] + uy[i][j] * cy_float[k]) / std::pow(cs,2) + pow((ux[i][j]*cx_float[k]+uy[i][j]*cy_float[k]),2)/(2*std::pow(cs,4)) - (ux[i][j]*ux[i][j]+uy[i][j]*uy[i][j])/(2*std::pow(cs,2)));;
        }
